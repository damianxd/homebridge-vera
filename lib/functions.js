    ...
    var debug = HAPnode.debug;
      ...
      module.checkCharacteristics = function(device, map){
        setInterval( function() {
          map.forEach( (property)=>{
            var vera = parseInt(functions.getVariable(device.id,property.vera));
      ...
      module.processall = function(verainfo)
      {
          var devices = verainfo.devices_full_list.filter(function(device){
              ...
              scenes = verainfo.scenes.filter(function(scene){
                var found = (!config.ignorescenes || (config.ignorescenes && config.ignorescenes.indexOf(scene.id)<0));
                if (!found){
