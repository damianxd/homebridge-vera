    ...
    var debug = HAPnode.debug;
      ...
      module.cacheStatus = function(){
        return HAPnode
          .request({
            method:'GET',
            uri: 'http://'+config.veraIP+'/port_3480/data_request?id=sdata'
      ...
      module.getVariable = function(id, property, type){
        ...
        // console.log('checking:', id, property, type);
      ...
      module.checkCharacteristics = function(device, chars){
        let interval = setInterval( function() {
          chars.forEach( (property)=>{
            let characteristic = property.service.getCharacteristic(property.ios);
            let sending = characteristic.sending;
            let charName = characteristic.displayName;
            var latest = module.getVariable(device.id, property.vera, property.type);
            var current = characteristic.value;
            ...
            // If the latest is zero, then the device is off. Don't update the level, homekit will keep this
            ...
            if(sending){
              debug("%s %s", charName, sending);
              debug("iOS sending!");
              return;
            }

            if (latest !== current && !isNaN(latest) && !sending){
              debug("iOS sending?: %s", sending);
              debug("%s \x1b[36m%s\x1b[0m changed: \x1b[1m%s → %s\x1b[0m", device.name, charName, current, latest);
              
              let writeable = property.service.getCharacteristic(property.ios).props.perms.indexOf('pw') > 0;
              if(writeable){
                // device.preventRequest = true;
                // debug('PREVENT REQUEST FOR %s', device.name);
              }
      ...
      module.getVeraInfo = function()
      {
          return HAPnode
            .request({
              method: 'GET',
              uri: "http://" + config.veraIP + "/port_3480/data_request?id=user_data",
              json: true
            }).then( (data)=> {
              return HAPnode
                .request({
                  method: 'GET',
                  uri: "http://" + config.veraIP + "/port_3480/data_request?id=lu_sdata",
                  json: true
                }).then( (response)=>{
      ...
      module.executeAction = function(params){
          var url = 'http://'+config.veraIP+'/port_3480/data_request?id=lu_action&output_format=json'
